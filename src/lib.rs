...
pub mod ffi {
    ...
    unsafe extern "C++" {
        ...
        fn GetElementCoefficients(self: &VectorOfPolys, index: usize) -> Vec<String>;
    }
    ...
    unsafe extern "C++" {
        ...
        fn ExtractMatrixRow(matrix: &Matrix, row: usize) -> UniquePtr<Matrix>;
        fn ExtractMatrixCol(matrix: &Matrix, col: usize) -> UniquePtr<Matrix>;
        fn ExtractMatrixRows(matrix: &Matrix, startRow: usize, endRow: usize) -> UniquePtr<Matrix>;
        fn ExtractMatrixCols(matrix: &Matrix, startCol: usize, endCol: usize) -> UniquePtr<Matrix>;
        fn FormatMatrixCoefficient(matrix: Pin<&mut Matrix>);
    }
    ...
    unsafe extern "C++" {
        ...
        fn DCRTPolyDeserializeEvalMultKeyFromBytes(data: &[u8], serialMode: SerialMode) -> bool;
...
pub fn parse_coefficients_bytes(bytes: &[u8]) -> ParsedCoefficients {
    ...
    let coeff_count = u64::from_le_bytes([
    ...
    if offset + 8 <= bytes.len() {
        let mod_chunk_count = u64::from_le_bytes([
            bytes[offset],
            bytes[offset + 1],
            bytes[offset + 2],
            bytes[offset + 3],
