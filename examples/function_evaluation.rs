...
fn EvalLogisticExample() {
    ...
    _cc_params_ckksrns
        .pin_mut()
        .SetSecurityLevel(ffi::SecurityLevel::HEStd_NotSet);
    ...
    _cc_params_ckksrns
        .pin_mut()
        .SetScalingModSize(_scaling_mod_size);
    _cc_params_ckksrns
        .pin_mut()
        .SetFirstModSize(_first_mod_size);
    ...
    _input
        .pin_mut()
        .push(ffi::ComplexPair { re: -4.0, im: 0.0 });
    _input
        .pin_mut()
        .push(ffi::ComplexPair { re: -3.0, im: 0.0 });
    _input
        .pin_mut()
        .push(ffi::ComplexPair { re: -2.0, im: 0.0 });
    _input
        .pin_mut()
        .push(ffi::ComplexPair { re: -1.0, im: 0.0 });
    ...
    _expected_output.pin_mut().push(ffi::ComplexPair {
        re: 0.0179885,
        im: 0.0,
    });
    _expected_output.pin_mut().push(ffi::ComplexPair {
    ...
    _expected_output
        .pin_mut()
        .push(ffi::ComplexPair { re: 0.5, im: 0.0 });
    _expected_output.pin_mut().push(ffi::ComplexPair {
        re: 0.731064,
        im: 0.0,
    });
    ...
    println!(
        "Expected output\n\t[{} ]",
        _expected_output.iter().fold(String::new(), |acc, arg| acc
            + " "
            + "("
            + &arg.re.to_string()
            + ","
    ...
    println!(
        "Actual output\n\t[{} ]",
        _final_result.iter().fold(String::new(), |acc, arg| acc
            + " "
            + "("
            + &arg.re.to_string()
            + ","
...
fn EvalFunctionExample() {
    ...
    let _result = _cc.EvalChebyshevFunction(
        GetSqrt,
        &_cipher_text,
        _lower_bound,
        _upper_bound,
